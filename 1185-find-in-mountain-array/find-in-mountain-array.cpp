...
public:
    int findInMountainArray(int target, MountainArray &mountainArr) {
        ...
        while(end != start){
           ...
           if(mountainArr.get(peak) < mountainArr.get(peak + 1) ) start = peak + 1;
           else end = peak;
        }
        peak = start;
        ...
        while(start <= end) {
           ...
           else if(currentElement > target) end = targetIndex - 1;
        ...
        while(start <= end){
           ...
           int currentElement = mountainArr.get(targetIndex);
           if(currentElement == target ) return targetIndex;
           else if(currentElement > target) start = targetIndex + 1;
